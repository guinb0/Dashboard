...
def dashboard_geral():
    ...
    if risco_residual_por_modalidade:
        ...
        with col2:
            # CORREÇÃO: Utilizando a variável correta para o maior risco
            st.metric(
...
def main():
    ...
    # Se não está logado E não está tentando logar, mostrar tela de boas-vindas
    if not st.session_state.logged_in:
        st.title("Olá! Bem-vindo ao Sistema de Avaliação de Riscos")
        
        # Código HTML da página de boas-vindas
        html_code = """
        <!DOCTYPE html>
        <html lang="pt-BR">
        <head>
        ...
        # Espera a mensagem de "login_requested" para prosseguir
        message = st.session_state.get('html_message', None)
        if message == 'login_requested':
